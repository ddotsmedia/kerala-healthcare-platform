...
FEATURE: P-G7 — AI Usage Analytics
REPO PATH: /opt/kerala-healthcare-platform (VPS)
LOCAL: C:\websites\kerala-healthcare-platform


OBJECTIVE
AI assistant query patterns, unanswered questions,
RAG performance metrics.

NO NEW SCHEMA — uses ai_interaction_log.

ANALYTICS SERVICE
services/analytics/ai.js:
  getTopQueries(days=7)
  getUnansweredQueries():
    -- responses that contained "consult a doctor"
    -- (proxy for queries AI couldn't fully answer)
  getRAGHitRate():
    -- interactions where rag_source_ids is not empty

ADMIN
apps/admin/app/analytics/ai/page.js
  — Daily AI interactions count
  — Top query topics
  — RAG hit rate (% with source articles)
  — Unanswered query types
    → Used to identify knowledge gaps
    → Link to CMS: "Add an article about this"

Smoke tests:
  AI analytics shows interaction counts
  Unanswered queries identified
  RAG hit rate calculated
