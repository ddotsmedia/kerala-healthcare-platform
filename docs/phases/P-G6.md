...
FEATURE: P-G6 — Health Trend Reports
REPO PATH: /opt/kerala-healthcare-platform (VPS)
LOCAL: C:\websites\kerala-healthcare-platform


OBJECTIVE
District-wise health query trends, seasonal illness
patterns. Public health intelligence value.

NO NEW SCHEMA — aggregates from search_logs.

PAGES
apps/web/app/[locale]/health-trends/page.js
  — "Kerala Health Trends"
  — Most searched conditions this month
  — District-wise health search heatmap
    (text-based, not actual map)
  — Seasonal patterns: dengue spikes in monsoon,
    flu in winter
  — Note: "Based on anonymous search data"

ADMIN
  Aggregate search data weekly into trend summaries
  Display in admin analytics

Smoke tests:
  Trends page renders with data
