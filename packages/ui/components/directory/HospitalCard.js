...
// Optional `compare` prop adds a Compare checkbox (feeds the compare bar).

import VerificationBadge from './VerificationBadge.js';
import CompareToggle from '../compare/CompareToggle.js';
...
export default function HospitalCard({ hospital, locale = 'ml', compare = false }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition hover:shadow-md">
      <a href={`/${locale}/hospitals/${hospital.slug}`} className="block">
      ...
      {compare && (
        <div className="mt-3 border-t border-gray-100 pt-2">
          <CompareToggle hospital={{ id: hospital.id, name: hospital.name_en || hospital.name_ml, slug: hospital.slug }} locale={locale} />
        </div>
      )}
    </div>
  );
