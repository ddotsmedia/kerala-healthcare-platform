...
function render(locale, c) {
  ...
  if (locale === 'ml') {
    return {
      ...
      body: `${c.provider_name} എന്നയാളുമായുള്ള നിങ്ങളുടെ അപ്പോയിന്റ്മെന്റ് ${when} ${c.slot_date} ${c.slot_start}-ന്.`
    };
  }
  return {
    ...
    body: `Your appointment with ${c.provider_name} is ${when}, on ${c.slot_date} at ${c.slot_start}.`
  };
}
