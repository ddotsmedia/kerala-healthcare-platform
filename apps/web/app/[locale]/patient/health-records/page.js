...
export default async function HealthRecordsPage(props) {
  ...
  const [summary, allergies, medications, records] = await Promise.all([
    phrSummary(uid), listAllergies(uid), listMedications(uid), listRecords(uid)
  ]);

  return (
    <PHRDashboard locale={locale} summary={summary}
