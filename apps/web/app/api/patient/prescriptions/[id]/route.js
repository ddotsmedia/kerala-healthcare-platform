// PATCH  /api/patient/prescriptions/[id] — update metadata / medications
// DELETE /api/patient/prescriptions/[id] — soft delete
import { NextResponse } from 'next/server';
import { currentPatientId } from '@/lib/appointments';
import { updatePrescription, deletePrescription } from '@/lib/prescriptions';
...
export async function PATCH(request, ctx) {
  ...
  const r = await updatePrescription(uid, id, body);
...
export async function DELETE(request, ctx) {
  ...
  if (!uid) return err('unauthenticated', 401);
  const { id } = await ctx.params;
  const r = await deletePrescription(uid, id);
