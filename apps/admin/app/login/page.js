...
export default function AdminLogin() {
  ...
  async function requestOtp(e) {
    ...
    if (j.data?.sent) { setStage('code'); setMsg(j.data.debugCode ? `Dev code: ${j.data.debugCode}` : 'OTP sent'); } else setMsg('Could not send OTP');
  }
  async function verify(e) {
    ...
    if (r.ok) window.location.href = '/verification'; else setMsg('Invalid or expired code');
  }
  const inp = 'w-full rounded-lg border border-gray-300 px-4 py-2 text-base';
  const btn = 'w-full rounded-lg bg-brand px-4 py-2 font-medium text-white';
